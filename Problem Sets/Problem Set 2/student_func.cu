  ...
  // to sequential reference solution for the exact clamping semantics you should follow.
    ...
    for (int filter_r = -filterWidth/2; filter_r <= filterWidth/2; ++filter_r) {
      for (int filter_c = -filterWidth/2; filter_c <= filterWidth/2; ++filter_c) {
		...
		int image_r = fminf(fmaxf(r + filter_r, 0), static_cast<float>(numRows - 1));
        int image_c = fminf(fmaxf(c + filter_c, 0), static_cast<float>(numCols - 1));
                        ...
                        const int filterWidth)
{
  ...
  const dim3 gridSize(ceil(numCols/(float)numThreadsPerBlock), ceil(numRows/(float)numThreadsPerBlock), 1);
